...
export default function ChatbotWidget() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      // Reads environment variable set in .env.local
      const baseUrl = process.env.NEXT_PUBLIC_AI_API_BASE_URL || 'https://hisabdo-webapp-ai.onrender.com';
      ...
      // Handle 422 validation errors (e.g. empty string, invalid reference_date)
      if (res.status === 422) {
        const validationError = data.detail || 'Validation error: Invalid query or date format.';
        setMessages((prev) => [...prev, { role: 'assistant', content: `⚠️ ${validationError}` }]);
        return;
      }
      ...
      if (res.ok && data.status === 'ok') {
        ...
        const errorDetail = data.detail || data.message || 'Unable to process your request.';
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: 'Could not connect to the AI service. Please verify the server is active.' },
      ]);
    } finally {
      setIsLoading(false);
    }
